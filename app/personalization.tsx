...
export default function PersonalizationScreen() {
  ...
  const submit = async () => {
    ...
    if (touched.preferredCity) payload.preferredCities = cities;
  ...
  if (optionsQuery.isLoading) {
    return <Container scrollable className="bg-light-100 px-4 pt-6 dark:bg-dark-300"><View className="gap-2"><SkeletonBlock width="58%" height={24} /><SkeletonBlock width="100%" height={110} radius={16} /><SkeletonBlock width="100%" height={220} radius={16} /><SkeletonBlock width="100%" height={180} radius={16} /></View></Container>;
  }
  ...
  return <Container scrollable className="bg-light-100 px-4 pt-6 dark:bg-dark-300"><View className="gap-2 pb-10"><View className="gap-2"><Text variant="heading" weight="bold" rtlAlign="right">خصص تجربتك في جود</Text><Text size="xs" className="leading-6 text-gray-500 dark:text-gray-300">كل المعلومات اختيارية. اختر ما يناسبك الآن أو تخطَّ الخطوة وأكملها لاحقاً.</Text></View><Card padding="md" className="gap-3 border-gray-200 dark:border-dark-400"><Text size="sm" weight="semibold">كيف تستخدم جود؟</Text><View className="gap-2">{optionsQuery.data.intents.map((item) => <Choice key={item.value} selected={intent === item.value} label={item.label} onPress={() => { setIntent(item.value as UserIntent); setTouched((value) => ({ ...value, intent: true })); }} />)}</View></Card><Card padding="md" className="gap-3 border-gray-200 dark:border-dark-400"><Text size="sm" weight="semibold">ما المجالات التي تهمك؟</Text><View className="flex-row-reverse flex-wrap gap-2">{categories.map((item) => <Choice key={item.id} selected={categoryIds.includes(item.id)} label={item.name} onPress={() => { toggle(item.id, categoryIds, setCategoryIds); setTouched((value) => ({ ...value, interests: true })); }} />)}</View></Card>{showCapabilities ? <Card padding="md" className="gap-3 border-gray-200 dark:border-dark-400"><Text size="sm" weight="semibold">كيف يمكنك المساعدة؟</Text><Text size="2xs" className="text-gray-500 dark:text-gray-300">اختياري، ويفيد في مطابقة طلبات المساعدة المناسبة لقدراتك.</Text><View className="flex-row-reverse flex-wrap gap-2">{optionsQuery.data.capabilities.map((item) => <Choice key={item.id} selected={capabilityIds.includes(item.id)} label={item.name} onPress={() => { toggle(item.id, capabilityIds, setCapabilityIds); setTouched((value) => ({ ...value, capabilities: true })); }} />)}</View></Card> : null}<Card padding="md" className="gap-3 border-gray-200 dark:border-dark-400"><Pressable onPress={() => setIsCityModalOpen(true)} accessibilityRole="button" accessibilityLabel="اختر مدينة"><View pointerEvents="none"><Input label="مدينة" editable={false} showStatusIcon={false} rightIcon={<MapPin size={16} strokeWidth={2.25} />} value={cities.join("، ")} placeholder="اختر مدينة أو أكثر" fullWidth /></View></Pressable></Card><Button fullWidth disabled={isBusy} loading={completeMutation.isPending} onPress={submit}>حفظ ومتابعة</Button><Button fullWidth variant="tertiary" disabled={isBusy} loading={skipMutation.isPending} onPress={skip}>تخطي الآن</Button><Text size="2xs" rtlAlign="center" className="text-gray-500 dark:text-gray-300">التخصيص غير إلزامي، ويمكن لجود تحسين الاقتراحات تدريجياً من تفاعلك.</Text><SelectionModal visible={isCityModalOpen} title="اختر المدن" description="يمكنك اختيار أكثر من مدينة." options={cityOptions} multiple selectedValues={cities} onSelect={(value) => { setCities((current) => current.includes(value) ? current.filter((city) => city !== value) : [...current, value]); setTouched((state) => ({ ...state, preferredCity: true })); }} onClose={() => setIsCityModalOpen(false)} /></View></Container>;
}
