...
export function useReorderPostImages() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ postId, imageIds }: { postId: string; imageIds: string[] }) => postsApi.reorderImages(postId, imageIds), onSuccess: (data) => invalidatePostLifecycle(qc, data.id) }); }
...
export function useLikePost() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ postId, like }: { postId: string; like: boolean }) => like ? postsApi.like(postId) : postsApi.unlike(postId), onSettled: (_data, _error, variables) => { qc.invalidateQueries({ queryKey: postKeys.feeds() }); qc.invalidateQueries({ queryKey: postKeys.detail(variables.postId) }); qc.invalidateQueries({ queryKey: postKeys.mineLists() }); qc.invalidateQueries({ queryKey: postKeys.myDetail(variables.postId) }); } }); }
export function useLikeCampaign() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ campaignId, like }: { campaignId: string; like: boolean }) => like ? postsApi.likeCampaign(campaignId) : postsApi.unlikeCampaign(campaignId), onSettled: (_data, _error, variables) => { qc.invalidateQueries({ queryKey: postKeys.campaigns() }); qc.invalidateQueries({ queryKey: postKeys.campaign(variables.campaignId) }); qc.invalidateQueries({ queryKey: postKeys.feeds() }); } }); }
