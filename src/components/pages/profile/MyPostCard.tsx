...
import { Pencil, Trash2, Eye, Heart, MapPin } from "lucide-react-native";
...
export function MyPostCard({ post, authorName, authorUsername, onDelete }: Props) {
  ...
  return (
    <View className="mb-0 border-b border-gray-100 bg-transparent px-3 py-3 dark:border-dark-400">
        <View className="mb-3 flex-row-reverse items-start justify-between gap-3">
              ...
              <View className="mt-0.5 flex-row-reverse flex-wrap items-center gap-1">
                ...
                {post.city ? (
                  <View className="flex-row-reverse items-center gap-0.5">
                    ...
                    <Text size="2xs" className="text-gray-500 dark:text-gray-300">{post.city}</Text>
                  </View>
                ) : null}
              </View>
            </View>
          ...
          <View className={`rounded-full px-2.5 py-1 ${STATUS_CLASSES[post.status].split(" text-")[0]}`}>
            <Text
              ...
              weight="semibold"
              className={
                STATUS_CLASSES[post.status].includes("text-")
                  ? STATUS_CLASSES[post.status].substring(STATUS_CLASSES[post.status].indexOf("text-"))
                  : "text-primary-400"
              }
            >
        ...
        <View className="mb-2 self-end rounded-full bg-primary-400/10 px-2.5 py-1">
          <Text size="2xs" className="text-primary-400">{typeLabel}</Text>
        </View>

        <Text weight="semibold" size="sm" className="mb-2 text-right text-dark-100 dark:text-light-50">
          {post.title}
        </Text>
        {post.details ? (
          <Text size="sm" className="mb-2 text-right leading-6 text-dark-100 dark:text-light-50">
            {post.details}
          </Text>
        ) : null}
        ...
        {post.images.length > 0 ? (
          <View className="-mx-3">
            <FeedMediaGrid images={post.images} />
          </View>
        ) : null}

        {blockReason ? (
          <View className="mt-3 rounded-xl border border-error-200 bg-error-100/60 p-3 dark:bg-error-300/10">
        ...
        {post.status === "pending" ? (
          <View className="mt-3 rounded-xl bg-warning-100/70 p-3 dark:bg-warning-400/10">
            <Text size="xs" className="text-right text-warning-400">
              المنشور حالياً عند الإدارة للمراجعة، ولا يمكن تعديله قبل انتهاء المراجعة.
            </Text>
          </View>
        ) : null}
        ...
        <View className="mt-3 flex-row-reverse items-center justify-between pt-1">
          ...
          <View className="flex-row-reverse items-center gap-1">
            {canEdit ? (
              <Pressable
                onPress={() => router.push({ pathname: "/create-post", params: { postId: post.id } } as never)}
                className="size-9 items-center justify-center rounded-full bg-primary-100 dark:bg-dark-350"
                accessibilityLabel="تعديل المنشور"
              >
            ...
            <Pressable
              ...
              className="size-9 items-center justify-center rounded-full bg-error-100 dark:bg-error-300/10"
