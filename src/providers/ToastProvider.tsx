...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const hideToast = useCallback(() => {
    clearTimer();
    Animated.timing(opacity, { toValue: 0, duration: 140, useNativeDriver: true }).start(({ finished }) => { if (finished) setToast(null); });
  }, [clearTimer, opacity]);
  const showToast = useCallback((payload: ToastPayload) => {
    ...
    timerRef.current = setTimeout(hideToast, payload.duration ?? 1900);
  ...
  return (
    <ToastContext.Provider value={value}>
      <View className="flex-1">{children}{toast ? <View pointerEvents="box-none" style={{ position: "absolute", top: Math.max(insets.top, 6) + 6, left: 24, right: 24, zIndex: 9999, elevation: 9999 }}><Animated.View style={{ opacity, borderColor: accent, backgroundColor: isDark ? "#2E2F3A" : "#FFFFFF" }} className="flex-row-reverse items-center gap-2 rounded-xl border px-3 py-2 shadow-md"><View style={{ backgroundColor: `${accent}18` }} className="h-8 w-8 items-center justify-center rounded-lg"><Icon size={16} color={accent} strokeWidth={2.4} /></View><View className="flex-1"><Text weight="semibold" size="xs" className="text-dark-100 dark:text-light-50">{toast.title || DEFAULT_TITLES[toast.type]}</Text><Text size="2xs" className="mt-0.5 leading-4 text-gray-500 dark:text-gray-300">{toast.message}</Text></View><Pressable onPress={hideToast} className="h-6 w-6 items-center justify-center rounded-md" accessibilityRole="button" accessibilityLabel="إغلاق الإشعار"><X size={14} color={isDark ? "#D1D5DB" : "#6B7280"} strokeWidth={2.25} /></Pressable></Animated.View></View> : null}</View>
